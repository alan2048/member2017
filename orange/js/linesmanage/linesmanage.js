...
// 获得路线列表
function orangeLinesList_port(pageNum) {
    var data={
    ...
    initAjax(httpUrl.orangeLinesList,param,orangeLinesList_callback);
};
function orangeLinesList_callback(res) {
    if(res.code==200){
        ...
        for(var i=0;i<data.list.length;i++){
            ...
            data.list[i].studentJson=JSON.stringify(data.list[i].studentVOList);
        };
        ...
        $("#pagination").pagination({
            ...
            onPageClick: function (pageNumber, event) {
                orangeLinesList_port(pageNumber);
            }
        });
    };
};
