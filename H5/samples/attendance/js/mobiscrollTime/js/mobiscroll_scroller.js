...
function Scroller(elem, settings) {
        ...
        function read() {
            that.temp = ((input && that.val !== null && that.val != elm.val()) || that.values === null) ? parseValue01(elm.val()) : that.values.slice(0);
            that.setValue(true);
        }

        // 专门针对日期格式  解决年不自动匹配的bug
        function parseValue01(value) {
            if(value){
                var arr=value.split('-');
    ...
    var scrollers = {},
        ...
        defaults = {
            ...
            parseValue: function (value, inst) {
                var val = value.split(' ');
                var val01=val[0].split('-');
                var val02=val[1].split(':');
                var arr=val01.concat(val02);
                
                var ret=[];
                for(var i=0;i<arr.length;i++){
                    ret.push(Number(arr[i]))
                };
                return [2015,10,10,10,10];
            }
        },
