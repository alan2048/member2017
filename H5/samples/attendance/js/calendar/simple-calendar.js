...
var SimpleCalendar = function () {
  ...
  function SimpleCalendar(query, options) {
    ...
    var maxYear=new Date().getFullYear()+1;// 2016年至当前年的下一年
    //默认配置
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function updateEvent() {
      ...
      yearadd.onclick = function () {
        if(selectYear.value == new Date().getFullYear()+1){
          toastTip("提示","超出可查询数据范围");
        }else{
      ...
      monthadd.onclick = function () {
        if(selectYear.value == new Date().getFullYear()+1 && selectMonth.value ==12){
          toastTip("提示","超出可查询数据范围");
        }else{
          ...
          attendGetClassAttendanceInfo_port();// 查询接口  
        };
      };
      monthsub.onclick = function () {
        if(selectYear.value == 2016 && selectMonth.value ==1){
          toastTip("提示","超出可查询数据范围");
        }else{
          var currentmonth = selectMonth.value;
          var currentyear = selectYear.value;
